 ...
 */
import type { Fixture, Player, XPtsProjection } from "../../types/domain";

export interface XptsInputs {
  player: Player;
  fixture?: Fixture;
  isHome: boolean;
  /** Defaults to "v1-weighted". Bumped when the model is retrained. */
  modelVersion?: string;
}
...
export const projectXpts = ({
...
}: XptsInputs): XPtsProjection => {
  // Coarse heuristic weights — tuned by hand until the ML layer lands.
  const w = { fixture: 0.3, form: 0.3, underlying: 0.25, minutes: 0.15 };

  const fixtureDifficulty = fixture
    ? (isHome ? fixture.homeFdr : fixture.awayFdr) / 5
    : 0.5;
  ...
  const minutesPenalty =
    player.minutesRisk === "injured" || player.minutesRisk === "suspended"
      ? 0
      : player.minutesRisk === "doubtful"
        ? 0.5
        : player.minutesRisk === "rotation"
          ? 0.85
          : 1.0;
  ...
  const raw =
    w.fixture * fixtureFit +
    w.form * formWeight +
    w.underlying * underlyingStatsWeight;
